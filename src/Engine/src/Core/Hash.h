...
namespace BeeEngine
{
    ...
    concept PrimitiveType =
        std::is_same_v<T, bool> || std::is_same_v<T, char> || std::is_same_v<T, unsigned char> ||
        ...
        std::is_same_v<T, long long> || std::is_same_v<T, unsigned long long> || std::is_same_v<T, float> ||
        std::is_same_v<T, double> || std::is_same_v<T, long double> || std::is_same_v<T, char16_t> ||
    ...
    namespace Reflection
    {
        ...
        [[deprecated("Can not be used with Types, that use dynamic memory or other layers of indirection. Better write "
                     "custom Hash function in class")]]
        uint64_t Hash(const T& value, uint64_t seed = 0)
        {
            uint64_t hash = seed;
...
namespace std
{
    ...
    struct hash<T>
    {
        size_t operator()(const T& obj) const { return static_cast<size_t>(BeeEngine::Hash(obj)); }
    };
} // namespace std
