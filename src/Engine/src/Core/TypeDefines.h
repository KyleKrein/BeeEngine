...
namespace BeeEngine
{
    ...
    using ConstGet = std::conditional_t<Internal::prefer_pass_by_value<T>, T const, T const&>;

    using byte = std::byte;
    template <typename T>
        requires(!std::is_void_v<T>)
    using in = std::conditional_t<Internal::prefer_pass_by_value<T>, T const, T const&>;
