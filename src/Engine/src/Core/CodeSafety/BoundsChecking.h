...
constexpr inline void BeeAssertInBounds(auto&& x, auto&& arg, std::source_location location = std::source_location::current())
requires (std::is_integral_v<std::remove_cvref_t<decltype(arg)>> &&
          requires { std::ssize(x);})
{
    if consteval
    {
    ...
    {
        std::string message = "Index out of bounds: " + std::to_string(arg) + " not in [0, " + std::to_string(std::ssize(x)) + ")";
