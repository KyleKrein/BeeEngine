...
namespace BeeEngine::Internal
{
    ...
    WebGPUInstancedBuffer::WebGPUInstancedBuffer(in<WGPUVertexBufferLayout> shaderModule, size_t maxInstances)
        : m_GraphicsDevice(WebGPUGraphicsDevice::GetInstance()),
          m_Size(maxInstances * shaderModule.arrayStride),
          m_MaxInstances(maxInstances),
          m_OneInstanceSize(shaderModule.arrayStride),
    ...
    void WebGPUInstancedBuffer::Bind(void* cmd)
    {
        wgpuRenderPassEncoderSetVertexBuffer(
            (WGPURenderPassEncoder)(((RenderPass*)cmd)->GetHandle()), 1, m_Buffer.Buffer, 0, m_Size);
    }
