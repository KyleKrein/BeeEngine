...
namespace BeeEngine::Internal
{
    ...
    WebGPUGraphicsDevice::WebGPUGraphicsDevice(WebGPUInstance& instance)
    {
        ...
        DeletionQueue::Main().PushFunction([device]() { wgpuDeviceTick(device); });
    ...
    WGPUAdapter WebGPUGraphicsDevice::RequestAdapter(WGPUInstance instance, WGPURequestAdapterOptions const* options)
    {
        ...
        wgpuInstanceRequestAdapter(
            instance /* equivalent of navigator.gpu */, options, onAdapterRequestEnded, (void*)&userData);
    ...
    WGPUSurface WebGPUGraphicsDevice::CreateSurface(WGPUInstance instance)
    {
        ...
        WGPUSurfaceDescriptorFromWindowsHWND sfdHwnd = {
            .chain = {.next = nullptr, .sType = WGPUSType_SurfaceDescriptorFromWindowsHWND},
            .hinstance = nativeInfo.instance,
            .hwnd = nativeInfo.window};
        WGPUSurfaceDescriptor sfd = {.nextInChain = (WGPUChainedStruct*)&sfdHwnd, .label = "SDL Window"};
        ...
        if (wmInfo.subsystem == SDL_SYSWM_X11)
        {
            ...
            WGPUSurfaceDescriptorFromXlib sfdX11 = {
                chain : {next : null, sType : WGPUSType.SurfaceDescriptorFromXlib},
                display : x11_display,
                window : x11_window
            };
            WGPUSurfaceDescriptor sfd = {label : null, nextInChain : cast(const(WGPUChainedStruct)*) & sfdX11};
            surface = wgpuInstanceCreateSurface(instance, &sfd);
        }
        else
        {
        ...
        SDL_Renderer* renderer = SDL_CreateRenderer(
            window,
            "metal",
            /* WindowHandler::GetInstance()->GetVSync() == VSync::On ? SDL_RENDERER_PRESENTVSYNC : */ 0);
        if (renderer == nullptr)
        ...
        WGPUSurfaceDescriptorFromMetalLayer sfdMetal = {
            .chain = {.next = nullptr, .sType = WGPUSType::WGPUSType_SurfaceDescriptorFromMetalLayer},
            .layer = metalLayer};
        WGPUSurfaceDescriptor sfd = {.nextInChain = (WGPUChainedStruct*)&sfdMetal, .label = nullptr};
        surface = wgpuInstanceCreateSurface(instance, &sfd);
