...
namespace BeeEngine::Internal
{
    WGPUTexture WebGPUFrameBuffer::CreateTexture(FrameBufferTextureFormat format)
    {
        ...
        descriptor.usage = IsDepthFormat(format) ? WGPUTextureUsage_CopySrc | WGPUTextureUsage_RenderAttachment
    ...
    void WebGPUFrameBuffer::Resize(uint32_t width, uint32_t height)
    {
        ...
        // wgpuRenderPassEncoderSetViewport((WGPURenderPassEncoder)m_CurrentRenderPass.GetHandle(), 0, 0,
        // m_Preferences.Width, m_Preferences.Height, 0, 1);
    }
    ...
    void WebGPUFrameBuffer::Bind()
    {
        ...
        auto renderPass = wgpuCommandEncoderBeginRenderPass(
            ((WebGPUCommandBuffer*)&m_CurrentCommandBuffer)->GetHandle(), &descriptor);
    ...
    void WebGPUFrameBuffer::Unbind()
    {
        ...
        // wgpuRenderPassEncoderSetScissorRect((WGPURenderPassEncoder)m_CurrentRenderPass.GetHandle(), 0, 0,
    ...
    {
        BEE_PROFILE_FUNCTION();
        BeeExpects(m_Preferences.Width > 0 && m_Preferences.Height > 0 && m_Preferences.Width < 100000 &&
                   m_Preferences.Height < 100000);
        if (m_Initiated)
        {
        ...
        for (size_t i = 0; i < m_ColorAttachmentSpecification.size(); ++i)
        {
            ...
            m_ColorAttachmentsTextureViews[i] =
                CreateTextureView(texture, m_ColorAttachmentSpecification[i].TextureFormat);
        }
        if (m_DepthAttachmentSpecification.TextureFormat != FrameBufferTextureFormat::None)
        {
            ...
            m_DepthAttachmentTextureView =
                CreateTextureView(depthTexture, m_DepthAttachmentSpecification.TextureFormat);
        }
        m_Invalid = false;
    }
    /*
    ...
    void WebGPUFrameBuffer::ReadPixelsAsync(uint32_t attachmentIndex, uint32_t x, uint32_t y, uint32_t width, uint32_t
    height, const std::function<void(std::vector<uint8_t> data)>& callback)
    {
...
*/
    static uint32_t calculate_buffer_dimensions(uint32_t width, uint32_t alignment)
    {
        ...
        const uint32_t padded_bytes_per_row_padding = (align - unpadded_bytes_per_row % align) % align;
        const uint32_t padded_bytes_per_row = unpadded_bytes_per_row + padded_bytes_per_row_padding;
    ...
    int WebGPUFrameBuffer::ReadPixel(uint32_t attachmentIndex, int x, int y) const
    {
        ...
        if (!m_EntityIDBuffer)
            m_EntityIDBuffer =
                m_GraphicsDevice.CreateBuffer(WGPUBufferUsage_MapRead | WGPUBufferUsage_CopyDst, sizeof(float));
        ...
        bufferCopy.layout.bytesPerRow = calculate_buffer_dimensions(
            m_Preferences.Width, 256); // std::max(m_Preferences.Width, m_Preferences.Height) * sizeof(float) + 256 -
                                       // (std::max(m_Preferences.Width, m_Preferences.Height) * sizeof(float) % 256);
        bufferCopy.layout.rowsPerImage =
            m_Preferences.Height; // 1;//std::min(m_Preferences.Width, m_Preferences.Height);

        WGPUExtent3D extent{1, 1, 1};
        wgpuCommandEncoderCopyTextureToBuffer(
            ((WebGPUCommandBuffer*)&m_BufferCopyEncoder)->GetHandle(), &textureCopy, &bufferCopy, &extent);
        ...
        wgpuQueueOnSubmittedWorkDone(
            wgpuDeviceGetQueue(m_GraphicsDevice.GetDevice()),
            0,
    ...
    Task<int> WebGPUFrameBuffer::ReadPixel(uint32_t attachmentIndex, int x, int y) const
    {
        ...
        bufferCopy.layout.nextInChain = nullptr;
        bufferCopy.layout.offset = 0;
        bufferCopy.layout.bytesPerRow = calculate_buffer_dimensions(m_Preferences.Width,
    256);//std::max(m_Preferences.Width, m_Preferences.Height) * sizeof(float) + 256 - (std::max(m_Preferences.Width,
    m_Preferences.Height) * sizeof(float) % 256); bufferCopy.layout.rowsPerImage =
    m_Preferences.Height;//1;//std::min(m_Preferences.Width, m_Preferences.Height);

        WGPUExtent3D extent{1,1,1};
        wgpuCommandEncoderCopyTextureToBuffer(((WebGPUCommandBuffer*)&encoder)->GetHandle(), &textureCopy, &bufferCopy,
    &extent); m_GraphicsDevice.SubmitCommandBuffers(&encoder, 1);
        //BeeExpects(wgpuDeviceGetQueue(m_GraphicsDevice.GetDevice()) == m_GraphicsDevice.GetQueue());
