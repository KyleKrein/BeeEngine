...
namespace BeeEngine::Internal
{
    ...
    WebGPUMesh::WebGPUMesh(in<std::vector<Vertex>> vertices, in<std::vector<uint32_t>> indices)
        : m_GraphicsDevice(WebGPUGraphicsDevice::GetInstance()),
          ...
          m_IndexSize(indices.size() * sizeof(uint32_t)),
          m_Size(vertices.size() * sizeof(Vertex))
    {
        CreateVertexBuffers((void*)vertices.data());
    ...
    void WebGPUMesh::Bind(void* commandBuffer)
    {
        wgpuRenderPassEncoderSetVertexBuffer(
            (WGPURenderPassEncoder)(((RenderPass*)commandBuffer)->GetHandle()), 0, m_VertexBuffer, 0, m_Size);
        if (IsIndexed())
        {
            wgpuRenderPassEncoderSetIndexBuffer((WGPURenderPassEncoder)(((RenderPass*)commandBuffer)->GetHandle()),
                                                m_IndexBuffer,
