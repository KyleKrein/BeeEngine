...
namespace BeeEngine
{
    ...
    private:
        [[nodiscard]] static std::vector<uint32_t> CompileGLSLToSpirVAndCache(const Path& spvPath,
        ...
        static bool
        LoadSpirV(const Path& path, ShaderType type, out<std::vector<uint32_t>> spirv, out<BufferLayout> layout);
