...
namespace BeeEngine
{
    ...
    void NativeScriptFactory::RegisterScript(const char* name,
                                             NativeScriptFieldInfo* fields,
                                             unsigned long long int fieldsCount)
    {
        ...
        m_NativeScripts.emplace_back(
            NativeScriptInfo{stringName,
                             fieldsCount ? std::vector<NativeScriptFieldInfo>(fields, fields + fieldsCount)
    ...
    NativeScriptFactory::NativeScriptFactory(BeeEngineNativeScriptRegistryData& data) : m_Data(data)
    {
        BeeExpects(s_Instance == nullptr);
        s_Instance = this;
        m_Data.RegisterScript_HostOwned =
            [](const char* name, NativeScriptFieldInfo* fields, unsigned long long int fieldsCount)
