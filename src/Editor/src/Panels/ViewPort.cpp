...
{
    static auto PhysicalSize(auto size)
    {
    ...
    void ViewPort::HandleRmlUiEvents(Rml::Context* context, EventDispatcher& event)
    {
        BeeExpects(context);
        ...
        event.Dispatch<MouseMovedEvent>(
            [context](MouseMovedEvent& event)
            {
                return context->ProcessMouseMove(
                    PhysicalSize(event.GetX()), PhysicalSize(event.GetY()), RmlSDL::GetKeyModifierState());
            });
    ...
    void ViewPort::OnEvent(EventDispatcher& event) noexcept
    {
        ...
        if (m_GameContext && CurrentScene()->IsRuntime() && event.GetType() != EventType::MouseMoved &&
            IsMouseInViewport())
        {
            HandleRmlUiEvents(m_GameContext, event);
        }
    ...
    void ViewPort::UpdateRuntime(bool renderPhysicsColliders) noexcept
    {
        ...
        if (IsMouseInViewport())
        {
            if (m_GameContext)
            {
                ...
                HandleRmlUiEvents(m_GameContext, dispatcher);
            }
