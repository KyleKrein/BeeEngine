    ...
    public struct Box3 : IEquatable<Box3>
    {
        ...
        public float DistanceToNearestEdge(Vector3 point)
        {
            var distX = new Vector3(
                MathU.Max(0f, MathU.Max(_min.X - point.X, point.X - _max.X)),
                MathU.Max(0f, MathU.Max(_min.Y - point.Y, point.Y - _max.Y)),
                MathU.Max(0f, MathU.Max(_min.Z - point.Z, point.Z - _max.Z)));
            return distX.Length;
        }
        ...
        {
            return new { _min, _max }.GetHashCode();
        }
