...
// Общие данные для всех тестов
        ...
        {
            return BeeEngine::Internal::GeneralPurposeAllocator::s_Instance.FindBlockHeader(ptr);
        }
    };
    ...
    TEST_F(GeneralPurposeAllocatorTest, MemoryAllocationTest)
    {
        ...
        void *ptr = BeeEngine::Internal::GeneralPurposeAllocator::Allocate(size, 8);
        ASSERT_NE(ptr, nullptr); // Проверка, что память выделена успешно

        std::atomic<BeeEngine::Internal::AllocatorBlockHeader> *blockHeaderPtr = static_cast<std::atomic<BeeEngine::Internal::AllocatorBlockHeader> *>(FindBlockHeader(
                ptr));
        ...
        BeeEngine::Internal::GeneralPurposeAllocator::Free(ptr);
    ...
    TEST_F(GeneralPurposeAllocatorTest, AllocateAndFreeTest)
    {
        ...
        void *ptr1 = BeeEngine::Internal::GeneralPurposeAllocator::Allocate(size1, 8);
        ASSERT_NE(ptr1, nullptr) << "Memory 1 allocation failed.";
        std::atomic<BeeEngine::Internal::AllocatorBlockHeader> *blockHeaderPtr1 = static_cast<std::atomic<BeeEngine::Internal::AllocatorBlockHeader> *>(FindBlockHeader(
                ptr1));
        ...
        void *ptr2 = BeeEngine::Internal::GeneralPurposeAllocator::Allocate(size2, 8);
        ASSERT_NE(ptr2, nullptr) << "Memory 2 allocation failed.";
        std::atomic<BeeEngine::Internal::AllocatorBlockHeader> *blockHeaderPtr2 = static_cast<std::atomic<BeeEngine::Internal::AllocatorBlockHeader> *>(FindBlockHeader(
                ptr2));
        ...
        EXPECT_EQ(blockHeaderPtr1->load().Next(blockHeaderPtr1)->load().Previous(
                blockHeaderPtr1->load().Next(blockHeaderPtr1)), blockHeaderPtr1) << "Memory 1 link is incorrect.";
        EXPECT_EQ(blockHeaderPtr1->load().Next(blockHeaderPtr1), blockHeaderPtr2)
                            << "Memory 1 link to Memory 2 is incorrect.";
        EXPECT_EQ(blockHeaderPtr2->load().Previous(blockHeaderPtr2), blockHeaderPtr1)
                            << "Memory 2 link to Memory 1 is incorrect.";

        // Освобождение первого блока памяти
        BeeEngine::Internal::GeneralPurposeAllocator::Free(ptr1);

        // Освобождение второго блока памяти
        BeeEngine::Internal::GeneralPurposeAllocator::Free(ptr2);
        ...
        EXPECT_EQ(blockHeaderPtr1->load().size, blockHeaderPtr1->load().Next(blockHeaderPtr1)->load().previousSize)
                            << "BlockHeader 1 has incorrect size.";
    }
        ...
        std::thread thread1([&]()
                            {
                                ...
                                ASSERT_NO_THROW(GeneralPurposeAllocator::Free(ptr))
                                                            << "Memory free failed on first try.";
                            });
        int size = 500;
        void *ptr = GeneralPurposeAllocator::Allocate(size, 8);
        ASSERT_NE(ptr, nullptr) << "Memory allocation failed.";
