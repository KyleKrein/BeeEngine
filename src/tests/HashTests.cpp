...
TEST(HashTest, TestReflectableStructWithStrings) {
    ...
    uint64_t seed = 42;
    ASSERT_NE(Reflection::Hash(s1, seed), Reflection::Hash(s2, seed));  // хеши должны быть разными, потому что std::string - сложный тип, использующий динамическую память. с ним Reflection::Hash работает неправильно
    ASSERT_NE(Reflection::Hash(s1, seed), Reflection::Hash(s3, seed));  // хеши должны быть разными
}
